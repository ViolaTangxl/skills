...
2011 年，Netflix 开源了 Chaos Monkey，开启了混沌工程的先河。此后，这个理念逐渐被业界认可：与其等待生产环境出问题，不如主动注入故障，验证系统的弹性（Resilience）。AWS 在 2021 年推出了 Fault Injection Service (FIS)，让混沌工程变得更加标准化和易于管理。
...
本文以混沌工程为例，展示这种范式转变的具体实践：三个 Claude Code Agent Skill 如何让任何工程师都能完成 EKS 故障演练，从"先学 FIS 再做实验"到"描述意图就能做实验"。
...
从"我想测试 AZ 故障"到写出正确的 experiment template，新手需要走完：理解概念 → 选择 action → 了解兼容性 → 编写模板。这条路不短。
